...
GM.RoundLimit = CreateConVar("hmcd_roundlimit", 0, bit.bor(FCVAR_NOTIFY), "Количество раундов, которые мы должны сыграть перед сменой карты" )
GM.Language = CreateConVar("hmcd_language", "", bit.bor(FCVAR_NOTIFY), "Язык, которым ты будешь пользоваться" )
GM.MurdererWins=0
GM.BystanderWins=0
...
local function ThesePeopleAreFuckingIdiots(ply,cmd,args)
	...
	net.Send(ply)
	print(ply:Nick().." "..ply:SteamID().." отключил аберрацию прицела.")
end
concommand.Add("homicide_scope_fix",ThesePeopleAreFuckingIdiots)

local function ChangeDebugPrinting(ply,cmd,args)
	if((ply.IsAdmin)and not(ply:IsAdmin()))then ply:PrintMessage(HUD_PRINTTALK,"Ты не администратор.") return end
	if(args[1]=="1")then
		HMCD_DebugPrint=true
	elseif(args[1]=="0")then
		HMCD_DebugPrint=false
	end
	print("Убийство: отладочная печать включена.")
end
...
local function SpecSHTF(ply,cmd,args)
	...
	print("Режим выбран как "..tostring(GAMEMODE.SHTF_MODE_ENGAGED))
	ply:PrintMessage(HUD_PRINTTALK,"Режим выбран как "..tostring(GAMEMODE.SHTF_MODE_ENGAGED))
end
...
function GM:SpawnCop(pos)
	...
	Cop:SetBystanderName("Полицейский")
...
local ZombTypes={"npc_zombie","npc_zombie","npc_zombie","npc_zombie_torso","npc_fastzombie","npc_fastzombie","npc_fastzombie_torso","npc_zombine","npc_poisonzombie"}
...
function GM:Think()
	...
	if((CurTime()>SHTF_CheckTime)and not(self.SHTF_Specified))then
		...
		local Size,Threshold,Map=self:DetermineMapSize(),2500,game.GetMap()
		if(string.find(Map,"ttt_"))then Threshold=2000 elseif((string.find(Map,"mu_"))or(string.find(Map,"md_")))then Threshold=4000 elseif(string.find(Map,"zs_"))then Threshold=1300 end
		if(HMCD_CountAiNodes()>=100)then self.EnoughAINodes=true else self.EnoughAINodes=false end
		--print(Size)
	...
	if((self.PoliceTime<CurTime())and not(self.DEATHMATCH))then
		if not(self.PoliceNotified)then
			...
			for key,playah in pairs(player.GetAll())do
				if(self.SHTF)then
					playah:PrintMessage(HUD_PRINTTALK,"Прибыла Национальная гвардия!")
				else
					playah:PrintMessage(HUD_PRINTTALK,"Прибыла полиция!")
				end
			end
		end
		if(self.SHTF)then
	...
	for k,ply in pairs(player.GetAll()) do
		...
		if((ply:Team()==2)and(ply:Alive())and(self.DEATHMATCH))then
			if(self.RoundTime+20>CurTime())then
				if((self.RoundTime+20-CurTime())<10)then ply:PrintMessage(HUD_PRINTCENTER,"Битва начнётся через "..tostring(math.Round(self.RoundTime+20-CurTime()))) end
			end
		end
			...
			if not(ent.HmcdSpawned)then
				...
				if not(Good)then
					...
					if((Rep)and(math.Rand(0,1)<Rep[2])and(((Rep[3])and(self.SHTF))or not(Rep[3])))then
						...
						print("Убийство: анти-чит система удаляет "..tostring(ent))
						SafeRemoveEntity(ent)
					end
				end
			end
...
function GM:PlayerNoClip(ply)
	if((ply:GetMoveType()==MOVETYPE_NOCLIP)or(GetConVar("sv_cheats"):GetBool()))then
		...
		ply:PrintMessage(HUD_PRINTTALK,"Никаго тебе полёта с sv_cheats 0!")
	end
end
		...
		if((Att)and(IsValid(Att))and(Att.GetClass))then
			...
			if(Att:IsPlayer())then
				if((Att.Murderer)and not(Att==ent))then
					if(self.SHTF)then
						ent.LastAttackerName="Предатель"
					else
						ent.LastAttackerName="Убийца"
					end
				else
					ent.LastAttackerName=Att:GetBystanderName()
				end
			elseif(Class=="npc_metropolice")then
				ent.LastAttackerName="Полицейский"
			elseif(Class=="npc_combine_s")then
				ent.LastAttackerName="Солдат"
			elseif((Class=="prop_physics")or(Class=="prop_physics_multiplayer"))then
				ent.LastAttackerName="Предмет"
			elseif(dmginfo:IsFallDamage())then
				ent.LastAttackerName="Гравитация"
			end
...
concommand.Add("hmcd_version", function (ply)
	if IsValid(ply) then
		ply:ChatPrint("Убийство создано Jackarunda & Mechanical Mind версия " .. tostring(GAMEMODE.Version or "error"))
	else
		print("Убийство создано Jackarunda & Mechanical Mind версия " .. tostring(GAMEMODE.Version or "error"))
	end
end)
...
function HMCD_ElectricalArcEffect(pos,victim,scale)
	...
	for i=2,NumPoints do
		...
		while(WeCantGoThere)do
			...
			C_P_I_L=C_P_I_L+1;if(C_P_I_L>=200)then print("ПРЕДОТВРАЩЕНИЕ ВЫЛЕТА СЕРВЕРА") break end
		end
		...
		if(Dist>150)then
			...
			for i=2,NumPoints do
				...
				while(WeCantGoThere)do
					...
					C_P_I_L=C_P_I_L+1;if(C_P_I_L>=200)then print("ПРЕДОТВРАЩЕНИЕ ВЫЛЕТА СЕРВЕРА; Эффект дуги начался с объекта, отсекающего мир.") break end
				end
				PointTable[i]=NewPoint
...
concommand.Add("homicide_player_speed_mul",function(ply,cmd,args)
	...
	if(Num)then
		...
		print("Множитель скорости игрока: "..tostring(GAMEMODE.PLAYER_SPEED_MUL))
		ply:PrintMessage(HUD_PRINTTALK,"Множитель скорости игрока: "..tostring(GAMEMODE.PLAYER_SPEED_MUL))
	end
end)
...
concommand.Add("homicide_loot_spawn_mul",function(ply,cmd,args)
	...
	if(Num)then
		...
		print("Множитель спавна лута: "..tostring(GAMEMODE.LOOT_SPAWN_MUL))
		ply:PrintMessage(HUD_PRINTTALK,"Множитель спавна лута: "..tostring(GAMEMODE.LOOT_SPAWN_MUL))
	end
end)
