...
function GM:PlayerSpawn( ply )
	...
	if(DatTeam==1)then
		...
		if(ply.RoundsSpectator>=3)then
			if not(ply:IsListenServerHost())then ply:Kick("Слишком долго уши греешь") else ply:Kill() end
			return
		end
	else
	...
	for i=1,4 do
		timer.Simple(i*2,function()
			if((ply)and(ply:GetPhysicsObject()))then
	...
	timer.Simple(10,function()
		if((ply)and(IsValid(ply))and(ply:Alive())and(ply.Murderer))then
			...
			if(self.SHTF)then
				ply:PrintMessage(HUD_PRINTTALK,"Национальная Гвардия будет через "..tostring(mins).." минут(у/ы).")
			else
				ply:PrintMessage(HUD_PRINTTALK,"Полиция прибудет через "..tostring(mins).." минут(у/ы).")
			end
		end
	end)
		...
		if(self.SHTF)then
			if(self.DEATHMATCH)then
				...
				timer.Simple(math.Rand(1,5),function()
					if(IsValid(ply))then
						ply:SetHeadArmor("УБШ")
						ply:SetChestArmor("Уровень IIIA")
					end
				end)
			else
...
local HitLocationPhrases={
	[HITGROUP_HEAD]="В голову ▐",
	[HITGROUP_RIGHTARM]="В правую руку ▐",
	[HITGROUP_LEFTARM]="В левую руку ▐",
	[HITGROUP_LEFTLEG]="В левую ногу ▐",
	[HITGROUP_RIGHTLEG]="В правую ногу ▐",
	[HITGROUP_CHEST]="В торс ▐",
	[HITGROUP_STOMACH]="В живот ▐",
	[HITGROUP_GEAR]="",
	[HITGROUP_GENERIC]=""
}
...
function GM:DoPlayerDeath(ply,attacker,dmginfo)
	...
	local Typ=ply.LastDamageType or "Ты был ранен ▐"
	if(ply.LastBled)then
		ServerDeathString=ply:Nick().."Ты истёк кровью на смерть >>"
		DeathString="Ты истёк кровью на смерть >>"
	else
		ServerDeathString=ply:Nick().." (っಠ‿ಠ)っ "
		DeathString=" (っಠ‿ಠ)っ "
	end
	...
	if((Att)and not(Att==""))then
		local ServerAtt=Att
		if(Att==ply:GetBystanderName())then Att="Ты";ServerAtt="Ты" end
		ServerDeathString=ServerDeathString.."Виновник >>"..ServerAtt
		DeathString=DeathString.."Виновник >>"..Att
	end
	...
	if !ply.Murderer then
		...
		if IsValid(attacker) && attacker:IsPlayer() then
			...
			elseif attacker != ply then
				...
				if(ply.Innocent)then
					...
					msgs = Translator:AdvVarTranslate("{player} убил агрессивного нарушителя спокойствия.", {
						player = {text = attacker:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				end
				if(self.DEATHMATCH)then
					msgs = Translator:AdvVarTranslate("{player} убил {ded}", {
						player = {text = attacker:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)},
			...
			-- self:SendMessageAll("Случайный прохожий погиб при загадочных обстоятельствах.")
		else
		end
	else
		if attacker != ply && IsValid(attacker) && attacker:IsPlayer() then
			...
			if(RealName==GameName)then
				if(self.ZOMBIE)then
					msgs=Translator:AdvVarTranslate("{player} убил альфа зомби.", {
						player = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				elseif(self.SHTF)then
					msgs=Translator:AdvVarTranslate("{player} убил предателя.", {
						player = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				else
		...
		elseif(attacker!=ply && IsValid(attacker) && attacker:IsNPC())then
			local ct = ChatText()
			if(self.ZOMBIE)then
				ct:Add("Альфа-зомби был застрелен Национальной гвардией.")
			elseif(self.SHTF)then
				ct:Add("Предатель был застрелен Национальной гвардией.")
			else
				ct:Add("Убийца был задержан полицией.")
			end
			ct:SendAll()
		else
			local ct = ChatText()
			if(self.SHTF)then
				ct:Add("Предатель умер при загадочных обстоятельствах.")
			else
		...
		elseif(string.lower(text)=="*unstick")then
			...
			if(ply.Unstickable)then
				...
				if(((Wall)or(Playa)or(Still))and not(ply:InVehicle())and not(ply.HiddenInContainer))then
					...
					ply:PrintMessage(HUD_PRINTTALK,"Игрок застрял. Движемся...")
					...
					ply:PrintMessage(HUD_PRINTTALK,"Не удалось обнаружить застрявшего игрок.")
				end
			else
				ply:PrintMessage(HUD_PRINTTALK,"Вы уже использовали отклеиватель в этом раунде.")
			end
...
function GM:PlayerUse(ply,ent)
	...
	if(ent.ContactPoisoned)then
		if(ply.Murderer)then
			ply:PrintMessage(HUD_PRINTTALK,"Это отравлено!")
			return false
		else
...
function PlayerMeta:EnterContainer(ent)
	if((ent.PlayerHiddenInside)and(IsValid(ent.PlayerHiddenInside)))then
		self:PrintMessage(HUD_PRINTCENTER,"В этом контейнере уже кто-то есть.")
	else
...
function PlayerMeta:AntiCheat()
	...
	if(Weps)then
		for key,wep in pairs(Weps)do
			if not(wep.HomicideSWEP)then
				self:PrintMessage(HUD_PRINTTALK,wep:GetClass().." не допускается с sv_cheats 0!")
	...
	if(self:GetMoveType()==MOVETYPE_NOCLIP)then
		...
		self:PrintMessage(HUD_PRINTTALK,"Полёт не допускается с sv_cheats 0!")
	end
	if(self:HasGodMode())then
		self:GodDisable()
		self:PrintMessage(HUD_PRINTTALK,"Бессмертие не допускается с sv_cheats 0!")
	end
	local Health=self:Health()
	if((Health)and(Health>105))then
		self:SetHealth(100)
		self:PrintMessage(HUD_PRINTTALK,"Дополнительное Здоровье не допускается с sv_cheats 0!")
	end
	local Armor=self:Armor()
	if((Armor)and(Armor>0))then
		self:SetArmor(0)
		self:PrintMessage(HUD_PRINTTALK,"Броня не допускается с sv_cheats 0!")
	end
...
function PlayerMeta:MurdererHideIdentity()
	...
	if(GAMEMODE.SHTF)then self:SetBystanderName("Предатель") else self:SetBystanderName("Убийца") end
	...
	if(math.random(1,Chance)==1)then
		...
		elseif(DeathType==4)then
			...
			if not(self.Seizuring)then
				...
				net.WriteBit(true)
				net.Send(self)
				self:PrintMessage(HUD_PRINTTALK,"У тебя припадок.")
				self:PrintMessage(HUD_PRINTCENTER,"Конфискуем")
