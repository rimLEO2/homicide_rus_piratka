...
local function SendIdentity(data)
	...
	if(file.Exists("homicide_identity.txt","DATA"))then
		...
		if(#RawData==10)then
			...
			LocalPlayer():ChatPrint("Убийство: неправильное количество строк в homicide_identity.txt! Необходимо 10 строк")
		end
	end
end
...
local function FixGlitch(data) -- i hate you, garry
	if not(LocalPlayer().ConCommand)then return end
	print("Начало записи для того, чтобы исправить глюк с модедью. Обсудите это с Гарри.")
...
net.Receive("hmcd_help",function()
	...
	if(typ=="identity")then
		print("Чтобы использовать свою внешность на других серверах, перекинь identity.txt файл в папку garrysmod/garrysmod/data .\nТекстовый файл должен содержать десять строк, например:\nJimmyBob (имя)\nmale04 (модель)\n1 (красный цвет)\n0.6 (зелёный цвет)\n0.2 (синий цвет)\n100 (размер твоих бицепсов)\n80 (размер торса)\n105 (размер ног)\nnormal (одежда, может быть young, formal, cold, casual или normal)\nnerd glasses (украшение)\nЧтобы узнать какие есть аксессуары см. страницу мастерской и руководство по блогу Jackarunda, если вы не можете придумать имя.")
	end
end)
--]]
