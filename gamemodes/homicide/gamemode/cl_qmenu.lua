	...
	if radialOpen then
		...
		for k,ment in pairs(ments) do
			...
			if(ment.TransCode=="Drop Item")then
				Main="Скинуть Предмет";Sub="Выкинуть предмет из рук"
			elseif(ment.TransCode=="Drop Equipment")then
				Main="Сбросить Снаряжение";Sub="Скинуть изношенные предметы"
			elseif(ment.TransCode=="Drop Ammo")then
				Main="Сбросить Амуницию";Sub="Выбрать амуницию, чтобы сбросить"
			end
