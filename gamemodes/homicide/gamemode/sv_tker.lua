...
function PlayerMeta:KickForTeamKilling()
	...
	if not(self:IsListenServerHost())then
		...
		self:Ban(1,true)
		print("Игрок временно забанен:",Name,SteamID)
		for key,playah in pairs(player.GetAll())do playah:PrintMessage(HUD_PRINTTALK,Name.." был временно забанен за неоднократные нападения или убийства невинных людей.") end
	elseif(self:Alive())then
		for key,playah in pairs(player.GetAll())do playah:PrintMessage(HUD_PRINTCENTER,Name.." не может быть забанен , так как это создатель сервера") end
