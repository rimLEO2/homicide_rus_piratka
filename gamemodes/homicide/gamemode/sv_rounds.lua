...
function GM:EndTheRound(reason, murderer)
	...
	if(reason==4)then
		if murderer then
			...
			local msgs
			if(self.SHTF)then
				msgs = Translator:AdvVarTranslate("Убийца потерял волю к убийству, это был {murderer}.\nВсе жили долго и счастливо.", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			else
				msgs = Translator:AdvVarTranslate("Предатель потерял волю к убийству, это был {murderer}.\nВсе жили долго и счастливо.", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			end
	...
	elseif reason == 3 then
		if murderer then
			...
			local msgs
			if(self.SHTF)then
				msgs=Translator:AdvVarTranslate("Предатель покинул игру, это был {murderer}", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			else
	...
	elseif reason == 2 then
		...
		if(RealName==GameName)then
			if(self.SHTF)then
				msgs=Translator:AdvVarTranslate("Предателем был(а) {murderer}",{
		...
		local ct = ChatText()
		if(self.SHTF)then
			ct:Add("Невинные победили! ", Color(20, 120, 255))
		else
	...
	elseif reason == 1 then
		...
		local ct = ChatText()
		if(self.ZOMBIE)then
			ct:Add("Зомби выиграли!", Color(190, 20, 20))
		elseif(self.SHTF)then
			ct:Add("Предатели выиграли!", Color(190, 20, 20))
		else
	...
	elseif(reason==5)then
		...
		local col = murderer:GetPlayerColor()
		local msgs,RealName,GameName=nil,murderer:Nick(),murderer:GetBystanderName()
		msgs=Translator:AdvVarTranslate("{murderer} победил!", {
			murderer = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
		})
		local ct = ChatText()
		ct:AddParts(msgs)
	...
	elseif(reason==6)then
		msgs=Translator:AdvVarTranslate("Все погибли. Конец",{})
		local ct = ChatText()
		ct:AddParts(msgs)
		ct:SendAll()
	elseif(reason==7)then
		msgs=Translator:AdvVarTranslate("Матч окончен, время вышло",{})
		local ct = ChatText()
		ct:AddParts(msgs)
		ct:SendAll()
	end
	...
	print("Отчет об убийствах: убийца "..tostring(self.MurdererWins).." невинные "..tostring(self.BystanderWins))
...
function GM:StartNewRound()
	...
	if not(self.SHTF_MODE_ENGAGED)then
		...
		if(math.random(1,9)==3)then
			...
			local Chance=.9
			if(string.find(game.GetMap(),"zs_"))then Chance=.2 end
			if(math.Rand(0,1)>Chance)then
				if(self.EnoughAINodes)then
					self.ZOMBIE_MODE_ENGAGED=true
				else
					print("Убийство: на карте недостаточно узлов AI для Вспышки Зомби")
				end
			end
		end
	end
	...
	for key,dude in pairs(team.GetPlayers(2))do
		...
		if((self.ZOMBIE)and(dude.Murderer))then
			dude:SetBystanderName("Альфа Зомби")
...
function GM:RotateMap()
	...
	print("[Убийство] изменение карты на " .. nextMap)
	local ct = ChatText()
