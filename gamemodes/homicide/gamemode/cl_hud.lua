...
function GM:DrawStartRoundInformation()
	...
	if(self.DEATHMATCH)then
		t1="Готовьтесь к бою"
		desc={"Битва начнется, когда таймер достигнет нуля."}
	elseif(self.ZOMBIE)then
		t1="Ты Выживший."
		desc={"Либо убей Альфа-зомби,либо убегай или ождиай военных."}
	elseif(self.SHTF)then
		t1="Ты Невинный"
		desc={"В твоей группе есть жадный предатель","Будь осторожен"}
	end
	...
	if LocalPlayer().Murderer then
		...
		if(self.ZOMBIE)then
			t1="Ты альфа-зомби"
			desc={"Веди свою орду зомби","Пожирай мозги людей."}
		elseif(self.SHTF)then
			t1="Ты Предатель"
			desc={"Убей всех и забери их вещи.","Не попадись"}
		end
		c = Color(190, 20, 20)
	end
	...
	if hasMagnum then
		...
		if(self.ZOMBIE)then
			t1="Ты Выживший."
			t2="с оружием"
			desc={"Убей Альфа-зомби, или","убей всех зомби, или","прячься, пока не придет армия."}
		elseif(self.SHTF)then
			t1="Ты Невинный"
			t2="с оружием"
			desc={"Найди и убей предателя","Защити невинных"}
		end
	end

	local Col=255*timeLeft^2
	local Col1,Col2,Txt=Color(Col,Col,Col,255),Color(Col,Col,Col,128),"Убийство: "
	if(self.SHTF)then
		if(self.DEATHMATCH)then
			Txt=Txt.."Бонусный Раунд"
			draw.SimpleText("Каждый сам за себя.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.ZOMBIE)then
			Txt=Txt.."Режим Вспышки Зомби"
			draw.SimpleText("Зомби-инфекция вспыхнула. Выжившие должны бороться против Альфа-зомби и его орды.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		else
			Txt=Txt.."Режим Чрезвычайного Положения"
			draw.SimpleText("Местное общество распалось после катастрофы, и в группе есть жадный, кровожадный предатель.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	else
		if(self.PUSSY)then
			Txt=Txt.."Режим Без Оружия"
			draw.SimpleText("Огнестрельное оружие в этих помещениях запрещено. Есть серийный убийца, но полиция быстро реагирует.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.EPIC)then
			Txt=Txt.."Режим Дикого Запада"
			draw.SimpleText("Убийца на свободе. У всех есть оружие, включая убийцу. Копы медлят с ответом.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.ISLAM)then
			Txt=Txt.."Режим Джихада"
			draw.SimpleText("На свободе сумасшедший-самоубийца, который пытается всех взорвать.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		else
			Txt=Txt.."Обычный Режим"
			draw.SimpleText("Как и в любой другой день, только серийный убийца на свободе.", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	end
...
function GM:DrawGameHUD(ply)
		...
		if(GetViewEntity()==LocalPlayer())then
			if IsValid(tr.Entity) && LocalPlayer().Murderer && tr.Entity:GetClass() == "prop_ragdoll" && tr.HitPos:Distance(tr.StartPos) < 60 and not(self.ZOMBIE) then
				if tr.Entity:GetBystanderName() != ply:GetBystanderName() || colorDif(tr.Entity:GetPlayerColor(), ply:GetPlayerColor()) > 0.1 then
					local h = draw.GetFontHeight("MersRadial")
					drawTextShadow("[E] Замаскироваться", "MersRadialSmall", ScrW() / 2, ScrH() / 2 + 80 + h * 0.7, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			elseif((IsValid(tr.Entity))and(table.HasValue(HMCD_PersonContainers,string.lower(tr.Entity:GetModel())))and(tr.HitPos:Distance(tr.StartPos)<60))then
				local h = draw.GetFontHeight("MersRadial")
				drawTextShadow("[ПКМ]+[E] Спрятаться внутри", "MersRadialSmall", ScrW() / 2, ScrH() / 2 + 80 + h * 0.7, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
	end
	...
	if shouldDraw != false then
		...
		if((ply.ChestArmor)and(ply.ChestArmor!=""))then
			local str="Броня: "..ply.ChestArmor
		...
		if shouldDraw!=false then
			...
			if(self.SHTF)then Name="Невинный" end
			if(self.DEATHMATCH)then Name="Боец" end
			if(self.ZOMBIE)then Name="Выживший" end
	...
	if self:GetRound() == 1 then
		...
		if(self.TKerPentalty!=0)then
			if(self.TKerUnShowTime>CurTime())then
				if(self.TKerPenalty==1)then
					...
					if(self.SHTF)then
						drawTextShadow("В следующем раунде вы будете невинным.", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					else
						drawTextShadow("В следующем раунде вы будете невинным.", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					end
					AlreadyDrawn=true
				elseif(self.TKerPenalty==2)then
					surface.SetDrawColor(10,10,10,50)
					drawTextShadow("В следующем раунде вы будете зрителем.", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					AlreadyDrawn=true
				end
			end
		end
		if not(AlreadyDrawn)then
			...
			if((Ply.InnocenceLost)and(Ply.InnocenceLost>CurTime()))then
				surface.SetDrawColor(10,10,10,50)
				drawTextShadow("Вы потеряли свою невинность.", "MersRadial", ScrW() * .5, ScrH()*.85, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
			end
		end
	end
end
...
function GM:OpenAmmoDropMenu()
	...
	if(#table.GetKeys(Ammos)<=0)then
		Ply:ChatPrint("У тебя нет аммуниции")
		return
	end
	...
	DermaPanel:SetTitle("Выкинуть Аммуницию")
