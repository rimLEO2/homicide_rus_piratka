...
function WRandom:Roll() -- now dis is some magic shit hier, son
	...
	if((SERVER)and(HMCD_DebugPrint))then print("Убийца это "..tostring(Killa)) end
	for key,playa in pairs(Playas)do
		local Record=GAMEMODE.SHITLIST[playa:SteamID()] or 0
	...
	if((SERVER)and(HMCD_DebugPrint))then print("Стрелок это "..tostring(Gunna)) end
	if not(Killa)then print("Убийство: выбор убийцы провалился!");Killa=Playas[1] end
	if not(Gunna)then print("Убийство: выбор стрелка не удался!");Gunna=Playas[2] end
	...
	for key,playa in pairs(Playas)do
		...
		if((Record>=GAMEMODE.GimpPunishmentThreshold)and(GetConVar("sv_cheats"):GetInt()==0))then
			playa:PrintMessage(HUD_PRINTTALK,"Твоя карама равна"..math.Round((Record/GAMEMODE.KickbanPunishmentThreshold)*100).."%. продолжишь РДМ'ить и ты будешь забанен.")
			playa.KillaChance=1
			playa.GunnaChance=1
		end
	end
