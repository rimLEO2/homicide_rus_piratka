...
SWEP.PrintName = "Игла с Ядом"
SWEP.Instructions	= "Это крошечный импровизированный шприц с очень маленькой, острой иглой и несколькими миллиграммами смертельного нейротоксина рыбы Фугу. Используйте его, чтобы убивать очень осторожно.\n\nЛКМ ,чтобы уколоть кого-то.\nОдноразовый\nТребует времени для воздействия на организм жертвы.\nДелать укол нужно в спину жертвы."
SWEP.Author			= ""
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.BobScale=2
...
function SWEP:AttackFront()
	...
	if((IsValid(Ent))and(Ent:IsPlayer()))then
		if(self:CanBackStab(Ent))then
			...
			self.Owner:PrintMessage(HUD_PRINTCENTER,"Ты должен находиться за спиной жертвы!")
		end
	else
