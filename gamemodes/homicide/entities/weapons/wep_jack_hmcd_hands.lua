...
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.Instructions	= "Это твои руки. Используйте их, как сочтёте нужным. Обороняйся, нападай на людей, перемещай объекты и т.д.\n\nЛКМ , чтобы поднять кулаки/ударить.\nR , чтобы убрать кулаки\nКогда кулаки опущены, нажмите ПКМ, чтобы нести вещи.\nКогда кулаки подняты, нажмите ПКМ, чтобы блокировать атаки."
			...
			if(Dist<self.ReachDistance)then
				if(tr.Entity.ContactPoisoned)then
					if(self.Owner.Murderer)then
						self.Owner:PrintMessage(HUD_PRINTTALK,"Это отравлено!")
						return
					else
