...
SWEP.PrintName = "Сотовый Телефон"
SWEP.Instructions	= "Это смартфон, который можно использовать для вызова полиции, в результате чего они прибудут раньше.\n\nЛКМ , чтобы набрать номер 911."
SWEP.Author			= ""
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.BobScale=3
		...
		timer.Simple(2,function()
			if((IsValid(self.Owner))and(self.Owner:Alive()))then
				...
				if(self.Owner.Murderer)then
					self.Owner:PrintMessage(HUD_PRINTTALK,"Ты притворяешься, что звонишь в полицию..")
				else
			...
			if(IsValid(self))then
				if not(self.Owner.Murderer)then
					if(GAMEMODE.SHTF)then
						if(DatTime)then
							if(DatTime>60)then
								self.Owner:PrintMessage(HUD_PRINTTALK,"Национальная гвардия будет здесь через "..math.ceil(DatTime/60).." минут(у/ы).")
							else
								self.Owner:PrintMessage(HUD_PRINTTALK,"Национальная гвардия будет здесь через "..math.ceil(DatTime).." секунд(у/ы).")
							end
						end
						for key,ply in pairs(team.GetPlayers(2))do
							if(ply.Murderer)then
								ply:PrintMessage(HUD_PRINTTALK,"Кто-то вызвал Национальную гвардию!")
							end
						end
					else
						if(DatTime)then
							if(DatTime>60)then
								self.Owner:PrintMessage(HUD_PRINTTALK,"Полиция будет здесь через "..math.ceil(DatTime/60).." минут(у/ы).")
							else
								self.Owner:PrintMessage(HUD_PRINTTALK,"Полиция будет здесь через "..math.ceil(DatTime).." секунд(у/ы).")
							end
						end
						for key,ply in pairs(team.GetPlayers(2))do
							if(ply.Murderer)then
								ply:PrintMessage(HUD_PRINTTALK,"Кто-то вызвал полицию!")
							end
						end
					end
				end
