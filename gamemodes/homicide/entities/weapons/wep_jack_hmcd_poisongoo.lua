...
SWEP.Instructions	= "Это крошечный пузырек приготовленный в домашних условиях, очень концентрированный алкалоид, получаемый из различных растительных источников. Это очень мощный яд , и его можно наносить на любой режущий или проникающий инструмент.\n\nЛКМ , чтобы открыть меню."
SWEP.Author			= ""
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.BobScale=2
...
if(CLIENT)then
	function SWEP:OpenTheMenu()
		...
		DermaPanel:SetTitle("Отравить оружие")
		...
		for key,wep in pairs(Poisonables)do
			...
			if(wep.Poisonable)then
				PButton:SetText("Отравить "..wep:GetPrintName())
			elseif(wep.AmmoPoisonable)then
				PButton:SetText("Отравить "..wep.AmmoName)
			end
