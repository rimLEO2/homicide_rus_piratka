...
SWEP.PrintName = "Гвоздодёр"
SWEP.Instructions	= "Это типичный молоток плотника. Используйте его, чтобы вбивать гвозди в предметы и строить конструкции.Вы также можете атаковать им других.\n\nЛКМ , чтобы ударить.\nПКМ , чтобы забить гвоздь.\nВы можете вбить гвоздь только в тонкий элемент из мягкого материала, который плотно перекрывает другую поверхность.\nГвозди более надёжнее, чем изолента."
SWEP.Author			= ""
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.BobScale=3
				...
				if((NewEnt)and((IsValid(NewEnt))or(NewEnt:IsWorld()))and not((NewEnt:IsPlayer())or(NewEnt:IsNPC())or(NewEnt==Tr.Entity)))then
					if(HMCD_IsDoor(Tr.Entity))then
						if(self.Owner:GetAmmoCount(self.AmmoType)>=3)then
							...
							self:SprayDecals()
							self.Owner:PrintMessage(HUD_PRINTCENTER,"Дверь заблокирована")
							umsg.Start("HMCD_AmmoShow",self.Owner)
							umsg.End()
							self.Owner:ViewPunch(Angle(3,0,0))
							self:DoBFSAnimation("midslash2")
							...
							self.Owner:PrintMessage(HUD_PRINTCENTER,"Нужно не меньше 3 гвоздей , чтобы заблокировать дверь.")
						end
					else
						...
						self.Owner:PrintMessage(HUD_PRINTCENTER,"Прочность Связи: "..tostring(Strength))
						umsg.Start("HMCD_AmmoShow",self.Owner)
						umsg.End()
						self.Owner:ViewPunch(Angle(3,0,0))
						self:DoBFSAnimation("midslash2")
