...
function SWEP:SCKInitialize()
	...
	if CLIENT then
		...
		if((IsValid(self.Owner))and(self.Owner.GetViewModel))then
			...
			if((IsValid(vm))and not(self.Owner:IsNPC()))then
				...
				if (self.ShowViewModel == nil or self.ShowViewModel) then
					...
					-- vm:SetColor(Color(255,255,255,1)) -- don't ever do this
					// ^ stopped working in GMod 13 because you have to do Entity:SetRenderMode(1) for translucency to kick in
					// however for some reason the view model resets to render mode 0 every frame so we just apply a debug material to prevent it from drawing
					-- vm:SetMaterial("Debug/hsv") -- don't ever do this
				end
			end
		end
...
if CLIENT then
	...
	function SWEP:SCKDrawWorldModel()
		...
		if((self.GetShouldHideWorldModel)and(self:GetShouldHideWorldModel()))then return end
		...
		for k, v in pairs( tab ) do
			if (v.type == "Model" and v.model and v.model != "" and (!IsValid(v.modelEnt) or v.createdModel != v.model) and 
				...
				if (IsValid(v.modelEnt)) then
					v.modelEnt:SetRenderOrigin(self:GetPos())
					v.modelEnt:SetRenderAngles(self:GetAngles())
